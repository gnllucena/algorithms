...
namespace Algorithms.Arrays
{
    ...
    // one team is designated the home team,
    // while the other team is the away team.
    ...
    // Specifically, results[i] denotes the winner of competitions[i], where a 1 in the results array means that
    ...
    {
        public static string Problem(List<List<string>> competitions, List<int> results)
        {
            ...
            {
                // home vs. away logic
                var index = results[i] == 1 ? 0 : 1;

                var team = competitions[i][index];
                ...
                if (hashmap[team] > maxPoint)
                {
                    maxPoint = hashmap[team];
                    result = team;
                }
            }
